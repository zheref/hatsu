...
| **[Illumi](claude/agents/illumi.md)** | **The long watch** — `en`'s step 6, when a landing watch must outlive the session that started it. Read-only through `nen watch until`, under the mandatory cap; he wakes Kurapika and acts on nothing. His frontmatter carries no write tools. | **PROVISIONED, not ratified** — `en`'s watch **only**; see below |
