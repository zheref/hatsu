...
> **`v0.34.0` — a tag records a build that landed.** `kagutsuchi` cuts one tag after a successful upload; `susanoo` only names the tag that is coming and still pushes nothing. Opt-in per repository, with the name the repository writes and a mandatory species prefix.
>
