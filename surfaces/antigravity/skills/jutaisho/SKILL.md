...
Jutaisho is the **last** step of [`/ren`](../ren/SKILL.md)'s turn and the step
...
```bash
nen parse jutaisho --grammar "at [<gate:G1|G1-M|G2|G3|G4|G5>]" --line "<the invocation, minus the /jutaisho prefix>"
```
...
> [`/izanami`](../izanami/SKILL.md) § 2 records for checker scripts. **This is not a defect and
...
- **Carries no delegation**, and being invoked from inside [`/ren`](../ren/SKILL.md) or
  [`/en`](../en/SKILL.md) does not lend it one.

## Hard limits

- **Never prompts for [`/aka`](../aka/SKILL.md).** `aka` is human-called (brief § 2). Offering
