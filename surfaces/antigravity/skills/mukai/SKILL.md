...
| 2 | **adversarial review** | [`/hanten`](../hanten/SKILL.md) | one reviewer subagent per applicable scope that still has cycle budget; Kurapika fixes or pushes back with a reason. The effort's `.nen/hanten/<branch-slug>.cycle.json` is reused; settling findings does not reset it |
...
- **2 before 3.** Review fixes need a focused checkpoint before anybody spends the slow suite on
  ...
  the effort's, not this Mukai run's; a later `/hanten` on this branch continues `used`/`max`.
...
| `reports.dir`, `.template`, `.captures`, `.retain` | `nen/workflow.json` | step 9 — [`/spiritual-message`](../spiritual-message/SKILL.md) `as landing` |
