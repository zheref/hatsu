---
name: izanami
description: Repeat a READ-ONLY task until a condition holds. Use when the maintainer invokes /izanami <task> until <condition>, or asks to watch, poll, or keep checking something until it changes. Observation only — it reads, reports and stops; it never writes to GitHub, never labels, never merges. For a loop that must act, use /izanagi, which requires an explicit iteration cap.
---
...
[`/izanagi`](../izanagi/SKILL.md), which requires an explicit cap in its grammar precisely
...
```
/izanami <task> until <condition>
/izanami until <condition>
