---
name: rikugan
description: Render one session of work as a rich HTML report — this last turn, then session-wide accomplished, challenges, not delivered, a change-highlighted architecture diagram, screenshots, the exact launch command, decisions — from the one fixed template, never as a markdown summary. Use when the maintainer invokes /rikugan [as turn|landing|final], asks to see the report, the turn report or the final report, or whenever /ren, /mukai or /en reaches its reporting step. This last turn answers only the last human request; 01–07 cover the whole session against the base, on every process or product effort. A mukai-triggered last turn also names corrections, local-check fallout, and half-run stops. Architecture delta is a session-wide structural diagram, never a file-line inventory. The landing variant adds the PR body and the readiness verdict; the final variant adds the tests run and the touched-file coverage, and is the only one kept as a dated file under Reports/. Not a gate event — it publishes a page and rings nothing.
---
...
> **Show me the session: what the last request did, and everything this effort has landed, fought,
> missed, and moved, plus the exact line that starts it.**
...
variants of one page**, not three reports: the eight sections below are always the eight sections,
in that order, and a variant only ever **adds**.

**00 is the last turn. 01–07 are the session.** That split is the page, on every repository this
plugin reports — a Hatsu process effort and a product effort alike. A later turn that rewrites
01–07 from only the latest request is a page that hides the rest of the sitting.
...
Use the evidence already produced by the owning phase. Ordinary turn reports say `impacted regression:
not due until mukai` and `coverage: not due until mukai` when those phases have not run. Include the
...
merge above → `report render --dry-run` (exit `0`) → `report render` (exit `0`, zero `{{`
remaining). Token count grows when this template names new extension slots; `--dry-run` is the
proof, not a remembered 40. The same run against the **unmerged** document refuses at exit
...
### This last turn — always the last human request
...
- When it is non-null, the page owes the three mukai blocks even if they are empty:
  ...
  2. **Local checks that brought work back** — which check (`/gyo` / `nen shu lint`, focused tests,
...
### Architecture delta — a diagram of what moved, never a path list
...
- **06 How to launch is the *exact* command** [`/amaterasu`](../amaterasu/SKILL.md) last ran
...
- **Never omits 00 This last turn**, never fills it from 01–03 by copying those lists, and
  never puts earlier-turn work on 00.
- **Never fills 01–07 from only the last request.** Those sections are the session. Dropping
  an earlier turn's landed work because this render is about a later request is the defect.
- **Never omits 03 Not delivered**, and never leaves a gap out of it to make a session read better.
- **Never renders Architecture delta as a file list or a line-count summary.** `files[]` is
  inventory. Nodes and edges are conceptual. A one-file-one-node mapping is the old defect.
