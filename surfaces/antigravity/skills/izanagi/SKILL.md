...
| **No progress** | **3 consecutive claimed acting iterations that change nothing** end the run, even below the cap. A loop repeating a no-op is not converging; it is burning the cap to reach the same place. Read-only observations that the composed task explicitly excludes from its acting iterations—En's quiet pending reads are the canonical case—do not enter this count | Pure judgment — no `nen` verb detects "no-op"; the skill compares each claimed iteration's before/after state itself |
