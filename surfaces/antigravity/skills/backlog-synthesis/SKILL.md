...
```
/backlog-synthesis <critical | high | medium | low | all> [<repo>]
```
...
[`/file`](../file/SKILL.md) § 1 and
[`/backlog-state`](../backlog-state/SKILL.md) § 1 for the exact refusal shape (an unknown
...
- **The row schema is thin by design** (`{issueNumber, title, labels, prNumbers[], createdAt}` —
  same shape [`/backlog-state`](../backlog-state/SKILL.md) § 3 already documents). Severity
...
This is the same verb [`/file`](../file/SKILL.md) § 3 uses for duplicate reconciliation,
...
— [`/build`](../build/SKILL.md). **Offer it; never start it.** Releasing into build is a
separate go-signal.
