...
#    roster, the same list `/agents` shows). Look for the literal id `/<persona>`.
```
...
`"adapted": "no /<persona> subagent type on this surface; § 9 adapter"` beside it.
...
- **Never answers § 3's question 2 with `ls`**, and never reports a persona as raised when the
  surface's agent registry does not carry `/<persona>` (§ 3).
