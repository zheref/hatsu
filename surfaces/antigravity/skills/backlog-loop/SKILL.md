...
(`/build <CODE>#<N>`) — everything from "is this an idea, an epic, or a routable child" through
"here is a `CON-32`-Ready PR or a G5 stop" is that skill's own engine, not restated here. Once a PR
exists, [`sharingan`](../sharingan/SKILL.md) (`/sharingan <CODE>#<N> to <G2|G4>`) takes it the rest of the
