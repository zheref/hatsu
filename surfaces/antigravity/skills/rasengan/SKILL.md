...
4. **An inner loop that is honest.** The declared inexpensive checks are run while authoring. When
   executable behavior changed and the repository has an explicit lane whose `test` row scopes the
   authored behavior, rasengan may run `nen shu test --lane <scoped-lane>` for feedback too. This is
   useful feedback, never the mandatory checkpoint and never the full regression or coverage gate.
...
row exercises that behavior and run it through [`/tsukuyomi`](../tsukuyomi/SKILL.md). A lane is scoped by its declared argv; Nen supports the route as:

```bash
nen shu test --repo <path> --lane <explicit-scoped-lane> [--dry-run]
```

There is no `--scope` selector and `iteration.checks` does not name a second, checkpoint-only list.
Never infer a lane from its name. Read its declaration and confirm the argv is focused. Rasengan may
execute this route for feedback. If no unambiguous scoped lane exists, record the missing declaration
or Nen capability for kokusen; do not type a raw runner command and do not substitute the full suite.
Prose-only, data-only, or other non-executable changes with no relevant runnable behavior report
`focused tests: not applicable` with that reason; Hatsu's own Markdown-only changes are this case.
...
- **Never lowers a bar to make a check pass.** Not by narrowing the check, not by deleting the
  ...
  longer measures anything ([`/byakugan`](../byakugan/SKILL.md) for the coverage
  ladder, [`/gyo`](../gyo/SKILL.md) for lint, and [`/tsukuyomi`](../tsukuyomi/SKILL.md)
...
- **Never runs the full regression suite or coverage.** Impacted unit, UI and integration tests
  belong to [`/kotoamatsukami`](../kotoamatsukami/SKILL.md) at
  [`/mukai`](../mukai/SKILL.md). Capture, measurement and gating belong to
  [`/byakugan`](../byakugan/SKILL.md), also at mukai. Linting is [`/gyo`](../gyo/SKILL.md),
  on every Ren turn — when `lint` is in `iteration.checks`, that entry is gyo. Focused tests belong
  to [`/tsukuyomi`](../tsukuyomi/SKILL.md).
...
- **Never claims its own green is the commit's** — the gate re-runs the checks over the tree it stages
  (§ 10).
- **Never substitutes a full-suite `test`, `ui-test`, or `coverage` row for a missing scoped-test
  declaration.** Name the missing route and hand it to kokusen.
