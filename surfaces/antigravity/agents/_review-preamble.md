...
description: NOT AN AGENT — the shared reviewer protocol every Hatsu reviewer includes by reference, never raised or routed to on its own. /hanten raises the personas; their files point here for the identity header, the classification, the handbooks, the finding shape, the budget rule and the refusals. The mirror generator has no concept of a shared file, so it mirrors this one AS A PERSONA into every surface's agent set and counts it there; nothing invokes it, and zheref/nen#223 is the fix.
model: flash
---
