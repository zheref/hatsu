---
name: futon
description: Take one whole severity band of a repo's backlog from open issues to PRs that have an actor driving them, then run the terminal step the maintainer typed. Use when the maintainer invokes /futon <repo>@<severity>[+] [then tag | then tag+fanout], or asks to build all the mediums, work the highs and cut a tag, or clear a severity band. Kurapika scopes backlog-loop's engine to one band; every PR this run produces is his own, because Hatsu carries no CI plane — done means CON-32 Ready and a per-PR merge prompt. Cuts only when a then clause asked for one, and the cut itself is getsuga's. Never merges main, never publishes a release.
---
...
[`/build`](../build/SKILL.md) § 3 makes, invoked per issue — and **Manipulator** while a PR
that authorship produced is being driven to its gate ([`/sharingan`](../sharingan/SKILL.md)'s engine).
...
[`/build`](../build/SKILL.md) → [`/tensho`](../tensho/SKILL.md) →
[`/en`](../en/SKILL.md), and the terminal a `then` clause typed is handed to
[`/getsuga`](../getsuga/SKILL.md) rather than performed here — the tag and the
release-proposal PR, whose own release unit is [`/susanoo`](../susanoo/SKILL.md)'s.

**Neither phase past the tag is reachable from here, and each is unreachable for its own reason.**
[`/kagutsuchi`](../kagutsuchi/SKILL.md) — a non-production upload, per target — **never runs
...
`tag+fanout`). [`/mugetsu`](../mugetsu/SKILL.md) — publication, per target, **G3** — runs only
...
`futon` is a **scoped, terminated [`/backlog-loop`](../backlog-loop/SKILL.md)** — not a second
...
[`/build`](../build/SKILL.md) invokes [`/sharingan`](../sharingan/SKILL.md): *"everything after the
...
> [`/build`](../build/SKILL.md) already established, for the identical structural gap, that
...
> its tracking issue (private) — the same declaration [`/build`](../build/SKILL.md) made and
...
```
/futon <repo>@<severity>[+] [then tag | then tag+fanout]
```

```bash
nen parse futon --repo <target repo's own checkout> "<the raw invocation, minus the /futon prefix>" [--self <owner/name>]
```
...
wrong the way [`/backlog-state`](../backlog-state/SKILL.md) found `nen parse`'s *generic* path
...
> build-only)`, exit `0`; `"BC@high+ then tag"` → `terminal: tag`, exit `0`. **`/futon BC@high` —
...
Kurapika's MODE instead** — the identical substitution [`/build`](../build/SKILL.md) § 3 makes:
...
[`/backlog-state`](../backlog-state/SKILL.md)/[`/backlog-board`](../backlog-board/SKILL.md)
...
verbs the target repository declares, exactly as [`/build`](../build/SKILL.md) § 5 lays them out
...
[`/build`](../build/SKILL.md) § 5 makes for its own identical hole.
...
directional, a veto only, never a promotion — exactly [`/sharingan`](../sharingan/SKILL.md) § 4's own
...
**A PR that stalls is driven by [`/sharingan`](../sharingan/SKILL.md)'s own engine** — its
...
[`/sharingan`](../sharingan/SKILL.md)'s own channel (reply/resolve/re-request), never a label fire.
...
[`/getsuga`](../getsuga/SKILL.md)'s own lane**, not restated or reimplemented here. `nen release preflight` and `nen
...
[`/getsuga`](../getsuga/SKILL.md) for the cut. **Fan-out is skipped, and its issues stay open** — say this explicitly: the
...
**`then tag+fanout`**: the same hand-off to `/getsuga`, which performs the cut **and** the
