...
[`/build`](../build/SKILL.md), [`/futon`](../futon/SKILL.md),
[`/getsuga`](../getsuga/SKILL.md), [`/backlog-loop`](../backlog-loop/SKILL.md),
[`/jujisho`](../jujisho/SKILL.md), [`/tensho`](../tensho/SKILL.md) and
[`/senkei`](../senkei/SKILL.md) all reach it too. Say which one is holding the run.

**P1 · Warm up.** [`/hatsu-warmup`](../hatsu-warmup/SKILL.md), unconditionally. Every step of this
...
| Typed | Read as |
|---|---|
| `/sharingan` | this checkout's branch, gate derived (§ 2) |
| `/sharingan to <G2\|G4>` | this checkout's branch, gate as typed |
| `/sharingan <CODE>` | that repository, this checkout's branch — **only** when the checkout *is* that repository |
...
| **None** | Stop. Say the branch has no open PR, and name [`/shibari`](../shibari/SKILL.md) (compose and open one) or [`/tensho`](../tensho/SKILL.md) (from a dirty copy). **Never open one** — sharingan drives PRs, it does not create them |
...
at [`/build`](../build/SKILL.md); a closed or merged PR still ends the run; readiness is still
