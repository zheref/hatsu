...
because the failure is unrelated". [`/jutaisho`](../jutaisho/SKILL.md) § 4 renders it:
...
than green ([`/tsukuyomi`](../tsukuyomi/SKILL.md) § 4), and **aka carries that word through
...
1. Reuse § 3's lint only when catch-up was a complete no-op. Any changed tree path — source, test,
   ...
   caught-up tree.
2. Run [`/tsukuyomi`](../tsukuyomi/SKILL.md) in **full-regression mode** over every
...
4. When the full-regression list includes `ui-test`, tsukuyomi invokes that declared suite exactly
   once and [`/kotoamatsukami`](../kotoamatsukami/SKILL.md) supplies its UI-specific result and
...
- **The whole aka run never runs unasked**, and **never prompts for itself** — no agent, skill,
  report or stop option proposes `/aka` (§ 1). A named composite may invoke only § 6 under the
