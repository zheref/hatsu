...
named target [`/amaterasu`](../amaterasu/SKILL.md) can launch from then on, and a device that has
...
```bash
nen parse jujutsu --grammar "pair <device>" --line "<the invocation, minus the /jujutsu prefix>"
```
...
The third is the one this skill exists to report properly, and at the pinned build it is the
one the declaration this skill writes now **says out loud** (§ 6's `readyWhen`).

> **A device that is PRESENT is not a device that is READY, and at the pinned build nen holds that rule
...
> letting the later one win. At the pinned build the answer is the per-target keys:
...
- **The key is a short, human name** — `iphone`, `sim`, `mac`, `pixel` — because it is what the
  maintainer will type at `/amaterasu`. It is not the device's name and not its identifier.
...
reports `ok nen/contract.json project (…)`. **At the pinned build nen PARSES the block rather than
...
5. **RETIRED at nen `0.5`: `nen/workflow.json` is validated.** `nen schema check --repo <path>` carries
   an `ok  nen/workflow.json` row at the pinned build, so a `launch.default` naming nothing is caught
...
- **Carries no delegation**, and being reached from [`/amaterasu`](../amaterasu/SKILL.md)'s
