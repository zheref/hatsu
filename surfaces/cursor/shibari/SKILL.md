...
Shibari is the **sixth and last step of [`/mukai`](../mukai/SKILL.md)**. Everything above it in
...
**It is the last step, not the finish.** The PR it opens is not ready; [`/en`](../en/SKILL.md)
...
> [`/rikugan`](../rikugan/SKILL.md)'s § 3 assembly, which has already done exactly that filter
...
**Pre-PR, the same images are data URIs in the landing report** ([`/rikugan`](../rikugan/SKILL.md)
...
3. **`nen shu evidence --base <ref>` does not exist** — `project.evidence` is preserved and unread
   ...
   [`/rikugan`](../rikugan/SKILL.md)'s § 3 assembly rather than derived a second time** (§ 2).
