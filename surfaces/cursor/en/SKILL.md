...
[`/izanami`](../izanami/SKILL.md) uses, run **single-shot per observation**, exactly as
...
|---|---|
| **a new review, comment or thread** | claim an acting cycle, then back to step 2 — [`/sharingan`](../sharingan/SKILL.md) addresses every inline and summary finding through its own channel |
