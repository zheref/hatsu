...
summary: Publish one release to one production destination — a store, the live environment — by running the repository's declared `release` row (`nen shu release`) and/or `nen shu deploy
---
...
when the maintainer typed `$mugetsu <target>` in this same turn: that completes their call and
prompts for nothing.
...
`$mugetsu <target>` may be asked about (P4). What it must never do is publish and explain
afterwards.
