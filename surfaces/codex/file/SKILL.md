---
name: file
description: File one well-formed, correctly-labelled, non-duplicate issue — reconciling it against the open backlog first. Use when the maintainer invokes $file <problem> <repo> or a bare $file (which files what the session has just been discussing), or asks to file, open, log or raise an issue. Kurapika searches for duplicates to amend, near-neighbours to fold into, and issues this supersedes, then presents ONE plan for ONE confirmation before anything is written.
---
