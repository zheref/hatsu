---
name: murasaki
description: Bring the branch up to date with its base, re-prove the merged tree with the declared iteration checks and the declared tests, and push the result — but only if the branch was already published. Use when the maintainer invokes $murasaki [from <base>], asks to catch this branch up and send the update, or whenever $mukai starts or $en finds the PR behind. Murasaki composes $ao and $tsukuyomi by name, restates none of their protocol, and hands a red merged tree to $rasengan to author the fix. It never squashes, never force-pushes, never first-publishes a branch, and never opens a pull request.
---
...
| 2 | **prove** | this file, § 5 — the declared `iteration.checks` — **then** [`$tsukuyomi`](../tsukuyomi/SKILL.md) | the checks, then the declared `tests.required` — on the merged tree, not on the tree before it. A red one is handed to [`$rasengan`](../rasengan/SKILL.md) to author the fix; **proving is not authoring**, and this step does neither on the other's behalf |
...
[`$tsukuyomi`](../tsukuyomi/SKILL.md) then runs every `tests.required` verb (and `tests.extra`)
...
- **Permitted:** everything [`$ao`](../ao/SKILL.md) and
  [`$tsukuyomi`](../tsukuyomi/SKILL.md) are each permitted, under their own authority, one at a
