...
summary: Reconcile a long backlog into a smaller one — group open issues that target the same rule/clause, machinery file or root cause, propose a consolidation plan as an iterable Artifact, and
---
...
[`$backlog-state`](../backlog-state/SKILL.md) § 1. An unknown token or an unregistered origin is
