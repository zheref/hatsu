...
summary: Run one request end to end as a turn — warm up on the first turn, author the change the request asks for, verify and commit it locally, launch the app, publish the report, ring the bell
---
...
nen parse ren --grammar "<request> [--profile <profile:fast|standard|thorough>]" \
  --line "<the request, minus the $ren prefix>"
```
...
| 6 | **bell** | [`$jutaisho`](../jutaisho/SKILL.md) | the rungs the workflow declares — and a stop only if one is genuinely due. **A report carrying a `DECIDE` ask makes this step `jutaisho at <ask.gate>`**, so its options reach the surface's picker in this turn, never the page alone ([`docs/WORKFLOW.md`](../../../docs/WORKFLOW.md) § 4) |
