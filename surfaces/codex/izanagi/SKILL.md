---
name: izanagi
description: Repeat a task that ACTS until a condition holds, under a mandatory iteration cap. Use when the maintainer invokes $izanagi <task> until <condition> up to <N>, or asks to keep doing something until it is done. The cap is required grammar, not a default — an invocation without "up to <N>" is asked for the cap, which is never defaulted or derived. Each iteration runs under its own authority; a human gate ends the loop rather than being retried past.
summary: Repeat a task that ACTS until a condition holds, under a mandatory iteration cap.
---
