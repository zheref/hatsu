---
name: amenotejikara
description: Swap which worktree the maintainer's core checkout holds, and swap back — list every checkout of the project with its branch, dirt and distance from the base, bring one worktree's committed tree into core so Xcode (or any IDE opened on core) and $amaterasu build and debug exactly that, and return core to where it started with its own uncommitted work restored. Use when the maintainer invokes $amenotejikara [list | <worktree|branch> [--take] | return | status], or asks to swap to a worktree, see what a worktree holds, debug a worktree from Xcode, or put core back. Core's work is parked in a pinned commit, never stashed; nothing is discarded, pushed or committed on anyone's behalf.
summary: Swap which worktree the maintainer's core checkout holds, and swap back — list every checkout of the project with its branch, dirt and distance from the base, bring one worktree's
---
...
```
$amenotejikara [list]                         # the default
$amenotejikara <worktree path | branch | worktree name> [--take]
$amenotejikara return
$amenotejikara status
```
