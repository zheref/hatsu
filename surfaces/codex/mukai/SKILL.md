---
name: mukai
description: Take a pushed branch to an open pull request — catch up on the base, run the adversarial review, run the tests and the UI tests, clear the coverage bar, collect the visual evidence, compose and open the PR, then start $en. Use ONLY when the maintainer invokes $mukai or asks in their own words to open the PR, put this up for review, or send it out for review. Mukai composes six skills by name and restates none of their protocol; the maintainer's call is the authorization for the PR and for the evidence mechanism's public step, and it is the last human call before the merge gate. Agents never propose it. It never merges.
---
...
| 3 | **prove it** | [`$tsukuyomi`](../tsukuyomi/SKILL.md) **and** [`$kotoamatsukami`](../kotoamatsukami/SKILL.md) where a `ui-test` verb is declared | the required suites, then the end-to-end/UI suite whose re-recorded snapshots become step 5's rows |
| 4 | **the coverage bar** | [`$gyo`](../gyo/SKILL.md) | touched-file line coverage against the ladder — **G5 under `coverage.minimum`** |
| 5 | **collect the evidence** | [`$kotoamatsukami`](../kotoamatsukami/SKILL.md)'s artifacts, laid out by [`$rikugan`](../rikugan/SKILL.md) `as landing` | the `UZF-26` table, one table per screen with states as columns |
| 6 | **compose and open** | [`$shibari`](../shibari/SKILL.md) | the body, the checks by verb, the PR from the last pushed commit, the reviewers |
...
`ui-test` verb, [`$kotoamatsukami`](../kotoamatsukami/SKILL.md) says so and step 5 has no rows
...
|---|---|---|
| `branch.base` | `nen/workflow.json` | step 1 — [`$murasaki`](../murasaki/SKILL.md), and step 6's PR base |
...
| `reports.dir`, `.template`, `.captures`, `.retain` | `nen/workflow.json` | step 5 — [`$rikugan`](../rikugan/SKILL.md) `as landing` |
