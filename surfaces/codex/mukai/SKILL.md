...
| → | **continue through the hand-over** | [`$en`](../en/SKILL.md) | the landing watch takes the open PR to verified Ready at G2/G4; the explicit Mukai run remains active until that hand-over reaches its terminus |
...
not Mukai's terminus.** It is the boundary where `en` and [`$sharingan`](../sharingan/SKILL.md)
...
- **Never claims readiness itself or by eye.** Mukai stays active while [`$en`](../en/SKILL.md)
