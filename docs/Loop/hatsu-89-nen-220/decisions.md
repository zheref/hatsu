...
| 2026-09-20 | `reports.sections` variants: `turn`, `turn-fast`, `landing` (rikugan), `final`, `register` (spiritual-message); `review.scopes`: code (Nobunaga, every path), security, architecture, ui, performance, release | ruled 2026-09-19: sections beside `reports.template`; the final report is a one-effort Spiritual Message |
