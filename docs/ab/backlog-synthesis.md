...
|---|---|---|
| 1 | "The repo is `bankai-core`" — hardcoded, no resolution step existed at all | `nen repo resolve <token>` / `nen repo resolve` (no token, cwd origin) — the same mechanism [`hatsu:file`](../file/SKILL.md) § 1 and [`hatsu:backlog-state`](../backlog-state/SKILL.md) § 1 already adopt; genuinely NEW here because the old skill never targeted anything but one fixed repo |
