...
(`<cache>\nen\v0.1.0\nen-windows-x64.exe`). Oracle/canon checkout: `<reference-repo>`,
...
| # | Old (prose) | New (`nen`) |
|---|---|---|
| 1 | "the repo's `bankai_scenario` (from `.github/workflows/bankai.yml`)" — the agent had to open that workflow file BY HAND and find the value | `nen repo scenario --repo <reference-repo checkout> --target <owner/name>` — reads the scenario **recorded** for the target in `schemas/repos.json` instead (§ 4 finding: this is not the same source the old prose named, though the registry is kept factual against the workflow file by hand elsewhere) |
| 2 | A static 2-row scenario→stack-folder table baked into the skill text: `swiftui-tca-uzf-v2` → `SW-{n}`, `compose-uzf-v2` → `KT-{n}`. Any other scenario had no entry at all — the old skill could not resolve `<scaffold-repo>` (scenario `<reference-repo>`) or a future `react-uzf-v1` consumer without an edit to its own prose | `nen canon resolve --stack-dir handbooks/stacks` derives `stacks/<scenario>/architecture.md` **directly from the scenario string**, for any scenario the registry names — verified live for all three real registry scenarios today (`swiftui-tca-uzf-v2`, `compose-uzf-v2`, `<reference-repo>`) plus the fourth handbook folder that already exists on disk (`react-uzf-v1`) with no consumer yet |
...
| 5 | The two-read-path split (`CON-13`): CI reads `<reference-repo>` live; build/local agents read a product repo's generated `.claude/rules/` mirror — a distinction the agent had to know and apply by context | Collapses to ONE path: `nen canon resolve --repo <reference-repo checkout>` computes the identical resolved set live and deterministically every time, so there is no separate mirror-trusting branch for a local session to apply |
| 6 | Missing/ambiguous rule → `bankai:handbook-question`, routed per `CON-37` to CI Yamamoto (handbook/schema gap) or Naruto (governance gap), landing a canon-lane PR `<reference-repo>` merges at G4 | **No verb replaces this** — it cannot, because `<reference-repo>` is frozen and neither destination exists to act on a PR there anymore. The ported skill (§ 4) turns this into "raise as a finding, stop at G5" instead — a genuine process change forced by the frozen state, not a mapping gap in `nen` |
...
All commands below ran against the real `<reference-repo>` checkout at its frozen tag. `nen repo
...
`schemas/repos.json` directly instead (the file `nen` itself reads): `<product-repo-A>`'s entry carries
`"scenario": "swiftui-tca-uzf-v2"`, `<product-repo-B>`'s carries `"scenario": "compose-uzf-v2"`,
`<scaffold-repo>`'s (the `consumers[]` entry, not its separate `maintained_tools` ownership entry)
carries `"scenario": "<reference-repo>"`. **Same, by construction** — `nen` reads the exact same field.
...
nen: '<reference-repo>' is not a consumer in <checkout>\schemas\repos.json. Its scenario cannot be
read from a registry that does not know it.
(exit 1)

$ nen repo scenario --repo <reference-repo checkout> --target <product-repo-C>
nen: '<product-repo-C>' is not a consumer in <checkout>\schemas\repos.json. Its scenario cannot be
read from a registry that does not know it.
(exit 1)

$ nen repo scenario --repo <reference-repo checkout> --target zheref/kro-pwa
nen: 'zheref/kro-pwa' is not a consumer in <checkout>\schemas\repos.json. Its scenario cannot be
read from a registry that does not know it.
(exit 1)
```

`<reference-repo>` is not a consumer of itself (it is the source of the canon, not a consumer of it);
`<product-repo-C>` and `zheref/kro-pwa` are recorded under `pending_onboarding`, not yet
...
$ nen repo scenario --repo <reference-repo checkout>
...
4), not routed around by hand: the skill instructs running `repo resolve` from inside the `<reference-repo>`
checkout.
...
$ nen canon resolve --repo <reference-repo checkout> --target <product-repo-A> --always-load "$ALWAYS" --stack-dir handbooks/stacks
...
$ nen canon resolve --repo <reference-repo checkout> --target <product-repo-B> --always-load "$ALWAYS" --stack-dir handbooks/stacks
scenario: compose-uzf-v2
always load: handbooks/uzf-core.md, handbooks/security-baseline.md, handbooks/ux-baseline.md, handbooks/release-policy.md, handbooks/quality-baseline.md
stack handbook: handbooks/stacks/compose-uzf-v2/architecture.md

$ nen canon resolve --repo <reference-repo checkout> --target <scaffold-repo> --always-load "$ALWAYS" --stack-dir handbooks/stacks
scenario: <reference-repo>
always load: handbooks/uzf-core.md, handbooks/security-baseline.md, handbooks/ux-baseline.md, handbooks/release-policy.md, handbooks/quality-baseline.md
stack handbook: handbooks/stacks/<reference-repo>/architecture.md
```

Also confirmed with `--json` (`<product-repo-A>`): `{"scenario":"swiftui-tca-uzf-v2","alwaysLoad":[...5 paths...],"stackHandbook":"handbooks/stacks/swiftui-tca-uzf-v2/architecture.md"}`.
...
- **Always-load, old prose:** `uzf-core.md`, `security-baseline.md`, `release-policy.md` (3 files —
  ...
  against the `<reference-repo>` history (`git -C <reference-repo checkout> log`/`show`, read-only):
...
- **Stack handbook, old prose (`<product-repo-A>`, `<product-repo-B>`):** the old skill's 2-row table maps
  ...
  **Same verdict** on the one part of the resolution the old table could actually reach.
  - **`<scaffold-repo>`: the old table has no row for scenario `<reference-repo>` at all.** Applied by
    hand, the old skill's prose simply cannot resolve this repo — there is nothing in its static table
    to match against. `nen canon resolve` resolves it correctly (`stacks/<reference-repo>/architecture.md`,
...
scenarios (`<reference-repo>`, and — by the same structural argument — `react-uzf-v1`, whose folder exists
...
```
$ nen canon resolve --repo <reference-repo checkout> --target <product-repo-A> --always-load "$ALWAYS"
nen canon: --stack-dir <dir> is required.
(exit 2)

$ nen canon resolve --target <product-repo-A> --always-load "$ALWAYS" --stack-dir handbooks/stacks
nen canon: <cwd>\schemas\repos.json: no such file. ... (exit 1 — --repo silently defaulted to cwd, it
did NOT refuse for being omitted)

$ nen canon resolve --repo <reference-repo checkout> --always-load "$ALWAYS" --stack-dir handbooks/stacks
nen canon: --target owner/name is required.
(exit 2)

$ nen canon resolve --repo <reference-repo checkout> --target <product-repo-A> --stack-dir handbooks/stacks
nen canon: --always-load <path,path,...> is required -- see 'nen canon resolve --help'.
(exit 2)

$ nen repo scenario --repo C:\nonexistent\path --target <product-repo-A>
...
- **The `CON-37` handbook-question routing has no live destination anymore**, and this is not a gap in
  `nen`'s verb surface — it is a consequence of `<reference-repo>` being frozen. Neither CI Yamamoto nor
...
1. **`nen repo resolve <token>` has no `--repo` flag and ignores `--from` for the token-supplied form**
   ...
   from inside the `<reference-repo>` checkout — but worth filing against `nen` itself: a `--repo` flag on
...
3. **`nen repo scenario`'s refusal text does not distinguish "never onboarded" from "recorded under
   ...
   `<reference-repo>` (not a consumer of itself, the canon source), `<product-repo-C>`, and
