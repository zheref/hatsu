...
All commands below ran **read-only** against the live `<reference-repo>` repository (issue/PR
...
```
$ git -C <reference-repo> show v0.11.3:scripts/ichigo_board.sh | head -20
```
...
| 7 | Object references (`RR-IS-#918`, `RR-PR-#925`) were typed by hand in the old table, `<CODE>-<TYPE>-#<N>`, with the state glyph/strikethrough applied by eye | `nen ref format --code --kind --number [--state] [--url]` — verified live (§ 3.4) |
...
$ nen repo resolve BS --repo <reference-repo checkout>
<scaffold-repo>  (BS)  via code
...
`BC (<reference-repo>)` as a known code. The origin-matching path only checks against
...
with `--repo` pointed at a checkout where `<reference-repo>` is asked to resolve **itself** — as opposed
to being asked about *from* a different repo's registry — still surfaces `BC` correctly in `all`
here because `<reference-repo>`'s registry lists itself under `product_codes`, just not under `consumers`/
...
registry would enumerate, not `<reference-repo>`'s own. Recorded here exactly as observed, not smoothed
...
### 2.3 — `nen gate derive`, two real `<reference-repo>` PRs
...
```
$ gh pr diff 925 --repo <reference-repo> --name-only
...
```
$ gh pr diff 916 --repo <reference-repo> --name-only
...
re-confirms the pointer is correct — `nen pr ready 925 --gh-repo <reference-repo> --gates
contracts/reference.gates.json --json` returns the same `not-ready: required checks reported but
...
$ nen pr fetch --target <reference-repo> --pr 927   # closed, unmerged
nen pr: could not fetch <reference-repo>#927 reviews: gh: Unprocessable Entity (HTTP 422)

$ nen pr fetch --target <reference-repo> --pr 916   # merged
nen pr: could not fetch <reference-repo>#916 reviews: gh: Unprocessable Entity (HTTP 422)

$ nen pr fetch --target <reference-repo> --pr 932   # merged
nen pr: could not fetch <reference-repo>#932 reviews: gh: Unprocessable Entity (HTTP 422)
...
```
$ nen color status --repo <reference-repo checkout> --present ready_g2_g4
🟢  ready_g2_g4  G2/G4-ready
precedence: on_hold > blocked > ready_g2_g4 > ready_g1 > in_progress

$ nen color status --repo <reference-repo checkout> --present ready_g2_g4,blocked
🔴  blocked  Blocked
outranked: ready_g2_g4

$ nen color status --repo <reference-repo checkout> --present blocked,on_hold
🔵  on_hold  Intentionally on hold
outranked: blocked

$ nen color status --repo <reference-repo checkout> --present ""
...
[{"id":"925","title":"Cancelled build leaves bankai:stage/building set",
  "refs":["RR-IS-#918","RR-PR-#925"],"gate":"G4","status":"🟠",
  "needs":"Kisuke addresses required checks"}]

$ nen board build --repo-slug <reference-repo> --rows-from rows.json --json > board.json
$ nen board render --board-from board.json
<reference-repo> -- generated 2026-09-02T00:59:42.717Z

| Effort                                           | Refs                   | Status (gate) | Needs                            |
| ------------------------------------------------- | ---------------------- | ------------- | -------------------------------- |
| Cancelled build leaves bankai:stage/building set | RR-IS-#918, RR-PR-#925 | 🟠 (G4)       | Kisuke addresses required checks |
```
...
```
$ nen ref format --repo <reference-repo checkout> --code BC --kind IS --number 929 --state open \
                 --url https://github.com/<reference-repo>/issues/929
📄 RR-IS-#929

$ nen ref format --repo <reference-repo checkout> --code BC --kind PR --number 916 --state merged \
                 --url https://github.com/<reference-repo>/pull/916
🔀 RR-PR-#916 ✓
...
```
$ nen backlog fetch --repo-slug <reference-repo> --limit 5
...
#939 [#925]  [Machinery] Nothing guards against bash 4 constructs, but unit_tests runs on macOS bash 3.2 — ${AGENT^} silently failed 8 tests on RR-PR-#925
#938  port pr_ready_gate.sh's plugin-cache guard classification into cli/src/ports/pr_ready_gate.ts (RR-IS-#733 plane parity)
...
$ nen backlog fetch --repo-slug <reference-repo> --limit 5 --json
{
  "repo": "<reference-repo>",
  "truncated": true,
  "rows": [
    {
      "issueNumber": 939,
      "title": "[Machinery] Nothing guards against bash 4 constructs, but unit_tests runs on macOS bash 3.2 — ${AGENT^} silently failed 8 tests on RR-PR-#925",
    ...
    {
      "issueNumber": 938,
      "title": "port pr_ready_gate.sh's plugin-cache guard classification into cli/src/ports/pr_ready_gate.ts (RR-IS-#733 plane parity)",
...
Both forms agree: 4 issue rows, `truncated: true` at `--limit 5` (the live `<reference-repo>` backlog
...
1. **`nen repo resolve` (no-token, origin-based form) cannot resolve a repository to itself when
   ...
   `<reference-repo>`'s own checkout (§ 2.1). The explicit-code form (`nen repo resolve BC --repo ...`)
...
- **The base-ref gate-assignment fix is new in this port**, carried in from a **live, open**
  finding against the very skill being ported — `<reference-repo>#929` ("Machinery leg of #879 —
