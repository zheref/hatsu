...
```
$ nen parse futon --repo <bankai-core> "@high"
...
```
$ nen parse futon --repo <bankai-core> "zheref/bankai-core@high+"
repo: zheref/bankai-core
...
```
$ nen parse futon --repo <bankai-core> "zheref/KroApple@high+"
repo: zheref/KroApple (KP)
band: high+ -> critical, high
terminal: (none -- build-only)
```
...
```
$ nen parse futon --repo <bankai-core> "@high+ then tag"
repo: zheref/bankai-core
band: high+ -> critical, high
terminal: tag
```
```
$ nen parse futon --repo <bankai-core> "@high+ then tag+fanout"
repo: zheref/bankai-core
band: high+ -> critical, high
terminal: tag+fanout
```
...
```
$ nen parse futon --repo <bankai-core> "zheref/bankai-core@high then tag"
repo: zheref/bankai-core
band: high -> high
terminal: tag
```

### 1.4 The terminal's scope rule generalizes to "the repo you are standing in," not hardcoded to `bankai-core`

```
$ nen parse futon --repo <bankai-core> "KP@high then tag" --self "zheref/KroApple"
repo: zheref/KroApple (KP)
band: high -> high
terminal: tag
```
...
```
$ nen parse futon --repo <bankai-core> "zheref/KroApple@high then tag+fanout" --self "zheref/KroApple"
repo: zheref/KroApple (KP)
band: high -> high
terminal: tag+fanout
```
The old skill's own prose read *"a `then` clause is valid on `bankai-core` ONLY, and is refused
...
every real session, since Kurapika's sessions stand in `bankai-core` when they type this. Not
exploited by this port (§ 8 of `SKILL.md` only ever hands `bankai-core`'s own release machinery to
...
```
$ nen parse futon --repo <bankai-core> "BC@medium"
nen: 'BC' resolves to 'zheref/bankai-core' in this registry's own product_codes, but no owner is
recorded for it (it names no consumer) and the checkout you are standing in ('zheref/bankai-core')
is not it. Run this from 'zheref/bankai-core''s own checkout, or name a consumer this registry
actually lists.
exit 2
```
Reproduced identically with a `then` clause, with `+`, and with `--self "zheref/bankai-core"`
...
```
$ nen parse futon --repo <bankai-core> "zheref/bankai-core@medium"
repo: zheref/bankai-core
band: medium -> medium
terminal: (none -- build-only)
```
```
$ nen parse futon --repo <bankai-core> "bankai-core@medium"
repo: zheref/bankai-core
band: medium -> medium
terminal: (none -- build-only)
```
...
```
$ nen parse futon --repo <bankai-core> "KC@high"
...
```
$ nen repo resolve BC   # run from inside the bankai-core checkout
...
narrower instance of it. **Corrected invocation for `bankai-core` until fixed**: `@<severity>`
(bare, when standing in the checkout) or `zheref/bankai-core@<severity>` / `bankai-core@<severity>`
— never `BC@<severity>`.

### 1.9 Refusals — bad severity, bad terminal, unresolvable token

```
$ nen parse futon --repo <bankai-core> "KP@urgent"
nen: 'urgent' is not a severity. Expected one of critical, high, medium, low, optionally suffixed '+'.
  try: KP@critical
exit 2

$ nen parse futon --repo <bankai-core> "KP@high then release"
nen: 'then release' is not a recognized terminal. Only 'then tag' and 'then tag+fanout' are accepted.
  try: KP@high then tag
exit 2

$ nen parse futon --repo <bankai-core> "ZZ@high"
...
$ nen parse futon --repo <bankai-core> "KP@high then tag"
nen: 'then tag' is refused against 'zheref/KroApple' -- the terminal is that repository's own
release machinery, and it is not the one you are standing in (zheref/bankai-core). Try the
...
```
$ nen parse futon --repo <bankai-core> "kp@HIGH+ then TAG"
nen: 'then tag' is refused against 'zheref/KroApple' -- ... (same self-mismatch as 1.9's last row)
  try: kp@high+
exit 2
```
...
```
$ nen parse futon --repo <bankai-core> "KP@high+" --json
{
...
```
```
$ nen parse futon --repo <bankai-core> "@medium" --json
{
  "repo": "zheref/bankai-core",
  "code": null,
  "isSelf": true,
...
$ nen backlog fetch --repo-slug zheref/bankai-core --limit 10 --json
```
...
**Full unfiltered severity counts**, fetched via `gh issue list --repo zheref/bankai-core --state
...
## 3. `nen label apply` — contract only (mutating; never exercised against `bankai-core`)
...
`outcome:"dry-run"`). Confirmed live that `bankai-core` carries the labels this port names
(`schema check`, § below) — never applied.

## 4. `nen pr ready` — live against both of `bankai-core`'s real open PRs

```
$ export GH_TOKEN=$(gh auth token)
$ nen pr ready BC#940 --repo <bankai-core> --gates <hatsu>/contracts/bankai-core.gates.json --explain
...
```
$ nen pr ready BC#925 --repo <bankai-core> --gates <hatsu>/contracts/bankai-core.gates.json --explain
...
- **`nen wake fire` / `nen wake verify`** — contract-inspected against `--help` only; never fired at
  `bankai-core` (mutating, forbidden by the shared brief). Not exercised by a real stalled review
