...
reads below ran **read-only** against the live `<reference-repo>` repository (its two real open
...
|---|---|---|
| 1 | `REPO=<owner>/<repo> scripts/pr_ready_gate.sh --verdict <N>` for readiness | `nen pr ready <CODE>#<N> --repo <path> --gates "$CLAUDE_PLUGIN_ROOT/contracts/reference.gates.json" --explain` — unchanged pointer from `pr-state`; see `docs/ab/pr-state.md` for that A/B, § 2.1 below for this port's own spot check |
...
| 8 | The wake channel itself: `bankai:wake/iterate` fired alone, a raw label add/remove via `gh` implied by prose | `nen wake fire --repo-slug <owner/name> --ref <CODE>-PR-#<N> --label bankai:wake/iterate --run` — contract inspected only (mutating; never fired at `<reference-repo>`, per the shared brief's boundary) |
...
verified live against real `<reference-repo>` PRs (rows 1, 2, 5, 6, 7, 11, plus the readiness composition
in row 4's stopgap), **3** are mechanized but exercised by contract-inspection only because they
mutate GitHub (rows 8–10), and **2 verbs this port needed do not work against real `<reference-repo>` PRs
...
Both open `<reference-repo>` PRs at run time:

```
$ gh pr list --repo <reference-repo> --state open --json number,title,headRefName,isDraft
...
$ nen pr ready 940 --gh-repo <reference-repo> --gates contracts/reference.gates.json --explain
...
**`RR-PR-#940` is a real `ready` PR — used as the worked example for § 2.5, § 2.8.** `RR-PR-#925` is
...
```
$ nen pr fetch --target <reference-repo> --pr 925
nen pr: could not fetch <reference-repo>#925 reviews: gh: Unprocessable Entity (HTTP 422)

$ nen pr fetch --target <reference-repo> --pr 925 --json
nen pr: could not fetch <reference-repo>#925 reviews: gh: Unprocessable Entity (HTTP 422)

$ nen pr fetch --target <reference-repo> --pr 940
nen pr: <reference-repo>#940: $.reviews -- expected an array, got object ({"id":5084846292,
...
$ nen pr fetch --target <reference-repo> --pr 940
nen pr: could not fetch <reference-repo>#940 reviews: gh: Unprocessable Entity (HTTP 422)
```
...
(`<migration-tracker>#38`, `<product-repo-A>#509`, `zheref/kro-pwa#86`, `<product-repo-B>#186`,
`<scaffold-repo>#21`, `#23`, `<product-repo-D>#22`) — none carries a lone `PENDING` review;
...
`<reference-repo>`-specific data, so no plausible flag combination routes around either.
...
> `zheref/akatsuki-ai`, `zheref/bankai-scaffold` — and **everything else on that axis is G2** — `G1`, `G1-M`, `G3` and `G5` are untouched — including a consumer repository declaring its own
...
```
$ gh pr diff 940 --repo <reference-repo> --name-only
...
$ nen gate derive --policy-paths "CONSTITUTION.md,handbooks/,agents/,schemas/" \
                  ...
                  --files "changelog.d/937-bc11-frozen-line-patch.md,cli/src/guards/bc11-allowlist.txt,cli/src/guards/bc11.repo.test.ts,cli/src/guards/bc11.test.ts,cli/src/guards/bc11.ts,handbooks/stacks/<reference-repo>/architecture.md"
G4
...
**First: no working override for `<reference-repo>`'s missing `schemas/gates.json`.**

```
$ nen pr next-blocker --target <reference-repo> --pr 925 --repo <reference-repo checkout> --reviewers sasuke,tenma,copilot
nen pr: C:\...\<reference-repo>\schemas\gates.json: no such file. Nen reads this repository's taxonomy
...
$ nen pr next-blocker --target <reference-repo> --pr 925 --repo <reference-repo checkout> \
    --gates contracts/reference.gates.json --reviewers sasuke,tenma,copilot
nen pr: C:\...\<reference-repo>\schemas\gates.json: no such file.   # identical -- --gates is silently ignored, not even flagged as unknown
```

**`--reviewers` (documented) does not substitute, and `--gates` (undocumented for this verb) is
silently accepted but has no effect** — confirmed `<reference-repo>`'s own checkout genuinely has no
...
$ cp contracts/reference.gates.json /tmp/nen-scratch-gates/schemas/gates.json

$ nen pr next-blocker --target <reference-repo> --pr 925 --repo /tmp/nen-scratch-gates
nen pr: could not fetch <reference-repo>#925 reviews: gh: Unprocessable Entity (HTTP 422)

$ nen pr next-blocker --target <reference-repo> --pr 940 --repo /tmp/nen-scratch-gates
nen pr: could not fetch <reference-repo>#940 reviews: gh: Unprocessable Entity (HTTP 422)
```
...
the identical live PR. **`nen pr next-blocker` cannot evaluate any real `<reference-repo>` PR today, with
or without the missing-taxonomy blocker patched around.** This port never calls it.

### 2.5 — `nen pr body-check`, a real PR body

```
$ gh pr view 925 --repo <reference-repo> --json body -q .body > pr925-body.txt
...
### 2.6 — `nen wake verify`, no `--run` — confirmed genuinely read-only, run against real `<reference-repo>`

```
$ nen wake verify --repo-slug <reference-repo> --now "2026-09-01T23:00:00Z" \
...
{"repo":"<reference-repo>","now":"2026-09-01T23:00:00Z","dryRun":true,"scanned":0,"results":[],"warnings":[]}
```
...
### 2.8 — `nen stop`, end to end with `RR-PR-#940`'s real data
...
real, frozen `<reference-repo>` repository by this port. Their full `--help` contracts (captured verbatim
...
1. **`nen pr fetch` is broken against real `<reference-repo>` PRs in (at least) two distinct ways in the
...
2. **`nen pr next-blocker` cannot evaluate any real `<reference-repo>` PR, for two compounding reasons.**
   ...
   `schemas/gates.json` inside `--repo`'s own checkout, which frozen `<reference-repo>` does not ship
...
3. **`nen pr next-blocker` has no documented flag for the `schemas/gates.json` substitute that `nen
   ...
   underneath would still leave `<reference-repo>` unjudgeable by this verb today.
...
`<reference-repo>`'s own CI-authored objects** (`#925`, `#940`, both carrying a `<!-- bankai
...
against the repository this skill actually lives in — it was validated against `<reference-repo>`, which is
