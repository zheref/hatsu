...
false`). **Enforcement is three-layered, and at the pinned nen `0.5.0` the third layer is the
...
1. **Probe** `nen --version` against the declared range. *Current pin, echoed for convenience:*
   `minimum: "0.5"`. **While nen's line is `0.x` that means `>=0.5.0 <0.6.0` exactly — a different minor is
