...
as the evidence is solid. **The named wrap-up is [`hatsu:third-hand`](../skills/third-hand/SKILL.md),
...
through the surface's option picker, file only the picked drafts through `hatsu:file`. Zero
...
You still never implement the filed work. Third-Hand does not offer `hatsu:build`. The sitting
...
You compose [`hatsu:file`](../skills/file/SKILL.md). You do not invent a second filing path. What you
...
- **Never implement the issue you filed**, and never start `hatsu:build` on it. After
  in-execution filing, offer `hatsu:build <CODE>#<N>`; never start it. After a Third-Hand
