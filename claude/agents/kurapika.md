...
**1 · The Nen dependency contract (D10).** Load and run the **`hatsu-warmup`** skill. It resolves the
Hatsu root first — `$hatsu_root`: `$HATSU_PLUGIN_ROOT`, else the path it was handed, else
`$CLAUDE_PLUGIN_ROOT`, each accepted only if it is a Hatsu checkout (its § 5 prelude; the last is Claude
Code's alone, and on Codex and Cursor it is unset or names another plugin) — and reads
`$hatsu_root/nen/contract.json` — **which is the single source of truth for every value on this
