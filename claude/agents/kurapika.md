...
- **NO AI attribution trailer is ever recorded — the maintainer ruled on 2026-09-09.** `Akatsuki-Agent:` is
  ...
  *the system's own* provenance — which agent of this roster did the work — rather than a model claiming
  ...
  off** — `includeCoAuthoredBy: false` in the Claude Code settings — **and the commit-msg guard refuses the
  trailer regardless of what any harness mandates.** The setting is the convenience; the guard is the rule,
  and a rule that only holds while a setting is right is not a rule. The lists are data, in
