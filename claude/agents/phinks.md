...
against the **exact commit proposed for the tag** — and **pre-PR through `hatsu:hanten`** on a
...
**Before the first test.** Resolve the scenario's pinned tooling through `hatsu:bankai-quality`, from the
