...
[`hatsu:getsuga`](../getsuga/SKILL.md) § 2 specifies — that skill is the authority for how, and this
...
[`hatsu:jutaisho`](../jutaisho/SKILL.md) § 4's rendering: `nen stop --who kurapika --gate G3
...
form and every mechanic is [`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md)'s — **the plan first, the
...
| `5` | the declared program could not be started | `nen shu tools --repo <path>` and relay the remedy — reading [`hatsu:susanoo`](../susanoo/SKILL.md) § 7 first. **nen never synthesises signing material**, and neither does this skill: a missing notarization or store credential ends the run |
...
5. **Nothing marks a destination as production.** `project.targets.<name>` carries no such field
   ([`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md) § Residue 1), so the split between that skill and
...
- **Not permitted:** cutting, moving or deleting a tag ([`hatsu:getsuga`](../getsuga/SKILL.md) cuts;
