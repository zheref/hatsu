...
[`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) and [`hatsu:byakugan`](../byakugan/SKILL.md) own
...
**Hand-back.** *Next in the wired run: `hatsu:mukai` — review, impacted tests, coverage, evidence and
...
several useful checkpoints with one red commit. It is [`hatsu:gyo`](../gyo/SKILL.md) — the
linting process — not the regression phase.

**Aka does not run tests.** Focused tests already ran at kokusen on every ren turn.
Project-wide tests are [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md)'s and coverage is
[`hatsu:byakugan`](../byakugan/SKILL.md)'s, at mukai. Linting is [`hatsu:gyo`](../gyo/SKILL.md)'s
...
#    the verb proves it is behind HEAD before it is allowed to be the squash point.
...
- **`ls-remote` answers the question that was actually asked.** *Is this branch on origin, and at
  ...
  > And a SHA that *is* present but is **not** an ancestor of `HEAD` is the worse case — a blind
  > reset would take it and silently drop every commit in between — which is why the verb's own
  > ancestry check runs before it resets and is § 4's third refusal below.
...
> nen wc squash --repo <path> --onto <ref> --message-file <message file>
> ```
>
...
>
> **This matters more in `aka` than anywhere else**, because § 4's commit is the squash's single
> commit: a refusal handed to it becomes the *only* message for every squashed commit,
...
1. **A dirty tree refuses.** `nen wc classify --repo <path> --base <base> --json` must report
   ...
   > This usually means a detached HEAD"* (verified live on a fixture; `docs/surfaces/evidence/surfaces.md` § 7, F5). That
   > is not a dirty tree and it is not a branch that needs cleaning: it is a checkout with **no branch to
   > publish**, which `aka` cannot invent — `nen wc publish` refuses a detached `HEAD` at exit `2`, there
   > being no ref to name, and the squash range in the table above has no `<branch>` to ask `origin` about.
...
> (`docs/surfaces/evidence/surfaces.md` § 8, F12). That was the old pin. **Do not present the trailer guard as
...
murasaki call [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) for impacted tests and
[`hatsu:byakugan`](../byakugan/SKILL.md) for coverage. Aka grants them no helper authority and no
test run.
...
and exits `1` with `needsForce: true` when the upstream is not an ancestor of `HEAD`.

- **Never a force form, in any spelling**: the verb has none to give, and § 4's range computation
  exists so that a plain push always suffices. **Exit `1` with `needsForce: true` is a finding to
  report, and the run stops there**: either something moved the remote branch under this run, or
  the squash folded a commit that was already published, and **a published commit is never
  rewritten** (`nen/decisions.json` row `rewrite-published-history`). Say which, and say nothing was
  pushed.
- **Never pushes `main`**, or any `branch.base`. The verb refuses the trunk at exit `2`; the
  `PreToolUse` guard in `hooks/hooks.json` refuses a raw push on the trunk at the harness level; this
  skill refuses it at the rule level, and all three are the answer to the same question.
...
4. **RETIRED at nen `0.13`: the push itself** is `nen wc publish [--set-upstream]` (§ 7).
...
- **The whole aka run never runs unasked**, and **never prompts for itself** — no agent, skill,
  report or stop option proposes `hatsu:aka` (§ 1).
...
- **Never hands `nen wc squash` a message file `nen commit format` did not exit `0` for**,
  and never merges the verb's two streams into that file (§ 4). In a squash, a refusal committed as
  the message is the whole effort's message.
