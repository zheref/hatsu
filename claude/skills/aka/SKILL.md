---
name: aka
description: Send the branch out — lint before rewriting history, squash only unpushed commits, bring the base underneath them, then own the full regression and instrumented-result capture on the final caught-up tree before pushing. Use ONLY on the maintainer's publish call. It exposes a reusable prepublication verification phase without lending composites its squash or first-publish authority.
---
...
1. If ao changed source or tests, rerun the declared lint row. A catch-up that changed only metadata
   may carry § 3's lint with the changed paths named.
...
4. Run [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) here when the required regression
   includes a declared `ui-test` suite. A re-recorded snapshot is a source/test change and therefore
   invalidates this run.

The regression must follow the final catch-up. Any later source or test change—including a review
fix, snapshot acceptance, test added by gyo, conflict resolution, or another catch-up—invalidates the
lint/regression evidence and returns here before publication. A red required suite is G5. An empty
configured set remains `not applicable — no tests configured`, neither green nor a stop.
