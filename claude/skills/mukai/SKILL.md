...
[`murasaki`](../murasaki/SKILL.md) — step 3 there, `nen wc publish` with no `--set-upstream`, no
squash, no force — is the whole of it. Aka is not called again. A branch found unpublished is a G5 ownership
...
it with proof,
[`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) says so and step 7 has no rows
...
| red required tests | [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md), step 4 | G5, same |
| touched-file coverage under `coverage.minimum` | [`hatsu:byakugan`](../byakugan/SKILL.md), step 5 | G5, same |
...
1. **The composition itself has no verb, and is not expected to get one.** nen owns operations, not
   ...
   **Mukai adds no residue of its own**, and every deterministic step inside the run is a verb or a
   named residue *in the skill that owns it* — `nen wc catch-up` and `nen wc publish` (steps 1 and 6,
   named in [`ao`](../ao/SKILL.md) and [`murasaki`](../murasaki/SKILL.md)), the Agent tool (step 2),
   ...
   `nen commit format --repo` gated on its own exit code and `nen commit write` (step 3, named in
   [`kokusen`](../kokusen/SKILL.md)), `nen shu evidence` (step 7), `nen pr open` and
   `nen pr edit-body` (step 8, named in [`shibari`](../shibari/SKILL.md)), and
   ...
   > 0.5*. **RETIRED at nen `0.13`**: the push, the commit and the PR are `wc publish`,
   > `commit write` and `pr open`; nothing here is by-hand git now.
...
- **Permitted, and only on the maintainer's own call:** everything the composed skills are each
  ...
  push, step 8's single `nen pr open` and, where the stack declares `public-mirror`, its own
