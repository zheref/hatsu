---
name: mukai
description: Take a pushed branch to an open PR that is genuinely ready at its human gate — catch up, review, measure coverage only from aka-captured instrumented results, remediate if needed, open the PR, then stay with En/Sharingan through required CI and review. Mukai owns coverage gating, never independently runs full regression, and gains no squash, first-publish, merge, or review-vote authority from nested phases.
---
...
> **The work is done and it is on the remote. Prove it, review it, show it, put it up, and stay until
> its current-head gate is Ready.**
...
| → | **continue through the hand-over** | [`hatsu:en`](../en/SKILL.md) | the landing watch takes the open PR to verified Ready at G2/G4; the explicit Mukai run remains active until that hand-over reaches its terminus |
...
report; **one** pull request with its body, its evidence and its reviewers; and an `en`/`sharingan`
handover that ends only when the pull request is verified Ready at its derived human gate.

**Never produces:** a merge, a gate label, a review vote, a tag, a release, a deploy. **Opening the PR is
not Mukai's terminus.** It is the boundary where `en` and [`hatsu:sharingan`](../sharingan/SKILL.md)
take over inside the same explicit run. The successful terminus is their current-head verdict:
`nen pr ready` says `ready`, `nen pr body-check` passes, the adversarial confirmation pass agrees,
and the gate is handed to the maintainer. A PR that merely exists, a screenshot publication that
completed, a required check that is still pending, an owed Copilot round, or an assistant choosing to
end its turn is **in progress**, never successful completion.

Pending remote work is held as observation through [`hatsu:en`](../en/SKILL.md), not returned to the
...
- **Never claims readiness itself or by eye.** Mukai stays active while [`hatsu:en`](../en/SKILL.md)
  obtains the `nen pr ready` + `nen pr body-check` + confirmation verdict; that verified handover is
  Mukai's successful terminus.
...
- **Never treats a response/session boundary as completion.** The handover to `en` is part of this
  explicit run. If the host actually interrupts the session, report the run as interrupted and resumable
  from fresh state; never rewrite an unfinished wait as a successful Mukai result (`en` §§ 6–7).
