---
name: mukai
description: Take a pushed branch to an open pull request — catch up on the base, run the adversarial review, run the tests and the UI tests, clear the coverage bar, commit and push what the review changed, collect the visual evidence, compose and open the PR, render the landing report, then start hatsu:en. Use ONLY when the maintainer invokes hatsu:mukai or asks in their own words to open the PR, put this up for review, or send it out for review. Mukai composes other skills by name and restates none of their protocol; the maintainer's call is the authorization for the PR and for the evidence mechanism's public step, and it is the last human call before the merge gate. Agents never propose it. It never merges.
---
...
|---|---|---|---|
| 1 | **catch up** | [`hatsu:murasaki`](../murasaki/SKILL.md) | the base underneath the branch, the build and the tests green on the result, pushed if the branch is published |
...
| 3 | **prove it** | [`hatsu:tsukuyomi`](../tsukuyomi/SKILL.md) **and** [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) where a `ui-test` verb is declared | the required suites, then the end-to-end/UI suite whose re-recorded snapshots become step 6's rows |
| 4 | **the coverage bar** | [`hatsu:gyo`](../gyo/SKILL.md) | touched-file line coverage against the ladder — **G5 under `coverage.minimum`** |
| 5 | **publish what the run changed** | [`hatsu:kokusen`](../kokusen/SKILL.md), then [`hatsu:aka`](../aka/SKILL.md) — or [`hatsu:murasaki`](../murasaki/SKILL.md)'s push half where the branch is already published | steps 2–4 edit the working copy; step 7 refuses to open a PR on commits `origin` has not got |
| 6 | **collect the evidence** | [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md)'s artifacts | the `UZF-26` table, one table per screen with states as columns |
| 7 | **compose and open** | [`hatsu:shibari`](../shibari/SKILL.md) | the body, the checks by verb, the PR from the last pushed commit, the reviewers |
| 8 | **the landing report** | [`hatsu:rikugan`](../rikugan/SKILL.md) `as landing` | rendered **after** step 7, because its two extra sections are step 7's outputs |
...
`ui-test` verb, [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) says so and step 6 has no rows
...
| red required tests | [`hatsu:tsukuyomi`](../tsukuyomi/SKILL.md), step 3 | G5, same |
...
| `commits.allowedAttributionTrailers` / `.forbiddenTrailers` | `nen/workflow.json` | step 5's message — [`hatsu:kokusen`](../kokusen/SKILL.md) |
...
> **`nen schema check` VALIDATES `nen/workflow.json` at the pinned `v0.6.0`** — verified live
