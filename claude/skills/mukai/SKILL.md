---
name: mukai
description: Take a pushed branch to an open pull request — catch up on the base, run the adversarial review, run the tests and the UI tests, clear the coverage bar, collect the visual evidence, compose and open the PR, then start hatsu:en. Use ONLY when the maintainer invokes hatsu:mukai or asks in their own words to open the PR, put this up for review, or send it out for review. Mukai composes six skills by name and restates none of their protocol; the maintainer's call is the authorization for the PR and for the evidence mechanism's public step, and it is the last human call before the merge gate. Agents never propose it. It never merges.
---
...
the reply, with **Manipulator** named alongside it while steps 5 and 6 run. Name the mode in play,
say when it switches and why, and never blend two under one header
...
| 3 | **prove it** | [`hatsu:tsukuyomi`](../tsukuyomi/SKILL.md) **and** [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) where a `ui-test` verb is declared | the required suites, then the end-to-end/UI suite whose re-recorded snapshots become step 5's rows |
| 4 | **the coverage bar** | [`hatsu:gyo`](../gyo/SKILL.md) | touched-file line coverage against the ladder — **G5 under `coverage.minimum`** |
| 5 | **collect the evidence** | [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md)'s artifacts, laid out by [`hatsu:rikugan`](../rikugan/SKILL.md) `as landing` | the `UZF-26` table, one table per screen with states as columns |
| 6 | **compose and open** | [`hatsu:shibari`](../shibari/SKILL.md) | the body, the checks by verb, the PR from the last pushed commit, the reviewers |
| → | **hand over** | [`hatsu:en`](../en/SKILL.md) | the landing watch takes the open PR to Ready and past the merge |

**The order is load-bearing in four places, and those four are the only ones mukai asserts:**
...
- **5 before 6.** The body's evidence table needs its rows. A body composed first would either
...
`ui-test` verb, [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) says so and step 5 has no rows
...
|---|---|---|
| `branch.base` | `nen/workflow.json` | step 1 — [`hatsu:murasaki`](../murasaki/SKILL.md), and step 6's PR base |
...
| `reports.dir`, `.template`, `.captures`, `.retain` | `nen/workflow.json` | step 5 — [`hatsu:rikugan`](../rikugan/SKILL.md) `as landing` |
| `project.evidence.globs` / `.scene` / `.mechanism` | `nen/contract.json` | steps 5 and 6 — the table's rows and how they reach the body |
...
finding count and how each was disposed.
...
**Not a prose recap of the landing report** — step 5 exists to replace exactly that, and duplicating
...
1. **The composition itself has no verb, and is not expected to get one.** nen owns operations, not
   ...
   **Mukai adds no residue of its own**, and every deterministic step inside the run is a verb or a
   ...
   (step 1, named in [`ao`](../ao/SKILL.md)), the Agent tool (step 2), `nen shu test` and the
   missing `nen shu test-report` (step 3), `nen shu coverage` and the missing `--touched` (step 4),
   the missing `nen shu evidence` (step 5), `gh pr create` and the missing `nen pr edit-body`
   (step 6, named in [`shibari`](../shibari/SKILL.md)).

## Authority

- **Permitted, and only on the maintainer's own call:** everything the six composed skills are each
  permitted, under their own authority, one at a time, in § 2's order — including step 6's single
  `gh pr create` and, where the stack declares `public-mirror`, its own evidence-publish step (§ 1).
