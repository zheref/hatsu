...
completeness live in [`hatsu:file`](../file/SKILL.md), [`docs/DISCOVERY.md`](../../../docs/DISCOVERY.md)
...
**P1 · Warm up, unconditionally.** [`hatsu:hatsu-warmup`](../hatsu-warmup/SKILL.md) — the contract's
...
| The **trunk**, clean, no session history | Stop. There is no sitting to harvest. Say so — **do not** harvest the repository in general, which is [`hatsu:file`](../file/SKILL.md)'s and [`hatsu:backlog-synthesis`](../backlog-synthesis/SKILL.md)'s work, not this skill's |
...
```
hatsu:third-hand [on <CODE>#<N>]
```

```bash
nen parse third-hand --grammar "on [<ref>]" --line "<the invocation, minus the hatsu:third-hand prefix>"
```
...
the definition — **do not pass `model`**, the same reconciliation [`hatsu:hanten`](../hanten/SKILL.md)
...
2. **File (after the pick).** Only the drafts the maintainer selected, through
   [`hatsu:file`](../file/SKILL.md) under Netero's completeness. Reconciliation still runs; a
...
After the answer, pass 2 files each selected draft through `hatsu:file` under Netero's
...
**Do not offer `hatsu:build` after a harvest filing.** File's § 6 offer starts a new effort; this
...
- **Permitted:** raise Netero under § 3; read the sitting's report and `.nen/` inputs; present
  the picker; file only picked drafts through `hatsu:file` under Netero's completeness.
- **Not permitted:** any merge; any review vote; any implementation of a filed issue; any
  `hatsu:build` started from this skill; any stage or release label; any push, tag, or deploy;
