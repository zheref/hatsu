...
[`hatsu:backlog-state`](../backlog-state/SKILL.md) § 1. An unknown token or an unregistered origin is
the trigger to ask, the registry's own codes as the options and an unregistered repository set up as
its `nen/repos.json` entry on the answer — never a fallback to `all` and never a guess.
