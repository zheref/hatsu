---
name: tenkai
description: Make a repository — new or existing, consumer or not — a working Hatsu consumer, deterministically and idempotently. It DIAGNOSES before it writes, reports per item what was already satisfied, what was missing, what had quietly drifted and what it changed, and re-runs safely on a repository that adopted three versions ago. Use when the maintainer invokes hatsu:tenkai [<path>], asks to adopt, onboard or set up a repository, asks why a Hatsu skill does not work somewhere, or when hatsu:hatsu-warmup's verification reports an adoption item unsatisfied. It writes no nen-owned declaration — those are routed to nen's own verb — never invents a colour taxonomy it was not given, never merges, never pushes, and never opens a pull request.
---
...
hatsu:tenkai check           # diagnose only, write nothing, whatever else is true
```
...
and not the offer — which is what [`hatsu:hatsu-warmup`](../hatsu-warmup/SKILL.md) § 0a uses.
...
5. the successor: **nothing**. Adoption is not a phase of the loop. A repository that Tenkai brought
   current is a repository [`hatsu:ren`](../ren/SKILL.md) can now be started in, and the hand-back
