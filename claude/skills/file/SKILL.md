---
name: file
description: File one well-formed, correctly-labelled, non-duplicate issue — reconciling it against the open backlog first. Use when the maintainer invokes hatsu:file <problem> <repo> or a bare hatsu:file (which files what the session has just been discussing), or asks to file, open, log or raise an issue. Also execute the shared discovery protocol when authorized work exposes a durable workflow, machinery, or owner gap. Kurapika searches for duplicates to amend, near-neighbours to fold into, and issues this supersedes before writing.
---
