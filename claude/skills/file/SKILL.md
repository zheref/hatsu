...
discovery. The normal explicit `hatsu:file` invocation still uses the reviewed plan in §4.
...
**Nothing is written before that answer** for an explicit `hatsu:file` request. Not the issue,
...
"filed". **A [`hatsu:third-hand`](../third-hand/SKILL.md) harvest pick replaces this banner for
...
**A filing from [`hatsu:third-hand`](../third-hand/SKILL.md) does not offer it** — that sitting is
