...
product code the registry lacks is set up through `nen/repos.json`'s own key.
...
> malformed stays `1`. Either is the trigger to ask for the code and set the entry up (§ 1), not the end.
...
>
> **Both are detectors now** — this skill and [`hatsu:kokusen`](../kokusen/SKILL.md) § 4 were each
