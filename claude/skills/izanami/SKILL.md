...
| `git fetch`, `git log`, `git diff`, `git status`, `git ls-tree`, `git show` | the mutating `git` family: `push`, `commit`, `merge`, `tag`, `rebase`, `reset`, `clean` |
...
`--command` (a push of `main`), and a 3-consecutive-error stop against a nonexistent PR number.
