---
name: kagutsuchi
description: Send a build to a declared NON-PRODUCTION destination — the internal channel, the testers' track, the preview environment — by running the repository's own `deploy` row through `nen shu deploy --target <name> --run`. Use ONLY when the maintainer invokes hatsu:kagutsuchi <target> or asks in their own words to upload this to a named destination. The plan is printed always, sending happens only on that call naming that target, no composite calls it and no agent proposes it. Production and the stores are hatsu:mugetsu at G3, never this skill.
---
