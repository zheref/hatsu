---
name: kagutsuchi
description: Send a build to a declared NON-PRODUCTION destination — the internal channel, the testers' track, the preview environment — by running the repository's own `deploy` row through `nen shu deploy --target <name> --run`. Use ONLY when the maintainer invokes hatsu:kagutsuchi <target> or asks in their own words to upload this to a named destination. The plan is printed always and sending happens only on that call, naming that target; no composite ever calls it, no agent ever proposes it, and one call sends to one target once. Production and the stores are hatsu:mugetsu at G3, never this skill.
---
...
authored the diff. [`hatsu:amaterasu`](../amaterasu/SKILL.md) starts an app *here* and is Transmuter
...
**P1 · Warm up.** [`hatsu:hatsu-warmup`](../hatsu-warmup/SKILL.md), unconditionally. The send is
...
the stores remain [`hatsu:mugetsu`](../mugetsu/SKILL.md) at `G3`, never this skill**, and no path
...
- **No composite ever calls it.** [`hatsu:ren`](../ren/SKILL.md),
  [`hatsu:mukai`](../mukai/SKILL.md), [`hatsu:en`](../en/SKILL.md),
  [`hatsu:futon`](../futon/SKILL.md) and [`hatsu:getsuga`](../getsuga/SKILL.md) **never** reach this
...
[`hatsu:susanoo`](../susanoo/SKILL.md) — the artifact it sends is one the repository's own `deploy`
row knows how to find, and if that row needs a package built first, **the maintainer runs
`hatsu:susanoo` and then this**. It never calls [`hatsu:mugetsu`](../mugetsu/SKILL.md), and a
...
[`hatsu:susanoo`](../susanoo/SKILL.md) § 5a — and a first line that does not is a reported refusal.
...
| `5` | the declared program could not be started | `nen shu tools --repo <path>` and relay the remedy — and read [`hatsu:susanoo`](../susanoo/SKILL.md) § 7 first: a green `tools` is not evidence about a program the toolchain block never declared |
...
**Then say nothing about what is available next.** Not `hatsu:mugetsu`, not another target, not "shall
...
4. **A record of the go.** Nothing in nen records that a maintainer authorised a target.
   ...
   [`hatsu:mugetsu`](../mugetsu/SKILL.md) § 3 states for the publication go.
