...
| [`mukai`](mukai/) | **The review-and-readiness phase — human-called.** `murasaki`¹ → `hanten`² → checkpoint/catch-up/aka verification³ → `gyo` coverage⁴ → final unchanged catch-up and update push⁵ → existing UI evidence⁶ → `shibari` PR⁷ → landing report and `en`⁸ through current-head readiness. An open PR or pending CI/review is progress, not its terminus. |
