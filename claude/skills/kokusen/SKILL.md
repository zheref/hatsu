---
name: kokusen
description: Commit the working copy locally, automatically, once the iteration checks are green — triage every path before staging it, ask on each flagged one, format the message through the Conventional-Commits verb, and sign it with `Hatsu-Agent` and nothing else. Runs as phase three of `ren` after `hatsu:rasengan`; invoke `hatsu:kokusen` by name to commit a checkpoint on demand. It never pushes, never adds an AI attribution trailer, and never commits a flagged file without an explicit yes or a secret at all.
---
...
1. **The checks are green FOR THIS TREE.** At the pinned nen `0.5.0` that is a verb, not a memory:
   ...
   | `1` | no proof, another lane's proof, or **the tree has moved** (both hashes printed) — also what a red build since the last green one looks like | run [`hatsu:rasengan`](../rasengan/SKILL.md), then check again |
   ...
   **It reports and blocks nothing** — no commit is refused and no file is written — so the refusal is
   ...
   forever (both verified live). Say so, run [`hatsu:rasengan`](../rasengan/SKILL.md) for the lane's
