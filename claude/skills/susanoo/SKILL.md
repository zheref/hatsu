...
**A build or distribution tag and [`hatsu:getsuga`](../getsuga/SKILL.md)'s release tag share one
...
[`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md) § 4a — and a `nameFrom` first line that does not is a
...
[`hatsu:getsuga`](../getsuga/SKILL.md) § 3 builds the release unit **through susanoo**, before its
...
reaches § 5a from exactly two places: inside [`hatsu:getsuga`](../getsuga/SKILL.md), where the tag
step is now skipped, and from the maintainer invoking `hatsu:susanoo` directly — which is a human
...
[`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md) § 1 refuses for the same reason.
...
A cut tag is **not** authorization to send anything. [`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md) is
still the maintainer's own per-target call and [`hatsu:mugetsu`](../mugetsu/SKILL.md) is still
