...
upload is [`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md) and a publication is
[`hatsu:mugetsu`](../mugetsu/SKILL.md); both are the maintainer's own per-target call, and neither is
this phase.
...
[`hatsu:getsuga`](../getsuga/SKILL.md) § 3 builds through it — and the maintainer may invoke it alone
...
**Susanoo is explicitly both**: it runs inside [`hatsu:getsuga`](../getsuga/SKILL.md) to build the
...
**Hand-back.** *Next in the wired run: `hatsu:getsuga`, which cuts the tag this unit belongs to.
Uploading and publishing are `hatsu:kagutsuchi` and `hatsu:mugetsu`, and both are yours alone to call.*
...
the iteration is what [`hatsu:rasengan`](../rasengan/SKILL.md) runs as its inner loop while authoring
and what [`hatsu:kokusen`](../kokusen/SKILL.md) runs at the commit gate, and an archive is neither.
...
missing lane or `archive` row is set up through `nen scaffold init`. **Signing material is never that
item** — a missing identity stays the declared precondition refusing (§ 4), supplied by the
maintainer through their own tool.
...
| `2` | usage, or an **unsatisfied precondition** — § 4 | the plan still prints, the failing rows marked `FAIL`. A usage gap (an unknown or missing lane) is asked for, never the end. Satisfy the precondition yourself and say which one it was; never route around it |
...
> [`hatsu:jutaisho`](../jutaisho/SKILL.md) § 4's rendering — `nen stop --who kurapika --gate G5
...
[`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md) § 4a, on a successful upload, that cuts it.
...
Announcing a tag is not authorization to send anything. [`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md)
is still the maintainer's own per-target call and [`hatsu:mugetsu`](../mugetsu/SKILL.md) is still
...
[`hatsu:getsuga`](../getsuga/SKILL.md) this block is the release unit's evidence and goes into the
release PR body.
...
- **Never uploads, deploys or publishes.** The verb is `archive`; `deploy` is
  [`hatsu:kagutsuchi`](../kagutsuchi/SKILL.md)'s and `release` is
  [`hatsu:mugetsu`](../mugetsu/SKILL.md)'s, and both are the maintainer's own per-target call.
