...
[`hatsu:rasengan`](../rasengan/SKILL.md) runs the iteration, and an archive is not one.
...
3. **Build proof.** `.nen/proof/<lane>.json` (`{treeHash, lane, exitCode, at}`) and
   ...
   forward. Same residue [`hatsu:rasengan`](../rasengan/SKILL.md) § 10 names for the build.
