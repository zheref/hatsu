...
**So typed alone, it is not refused — it is completed.** `hatsu:rasengan <what to author>` carries its own
request and runs exactly as `ren` step 2 does. A **bare** `hatsu:rasengan` has nothing to author: ask for
...
**It still commits nothing and pushes nothing** — [`hatsu:kokusen`](../kokusen/SKILL.md) holds the index —
and its inner-loop checks are still the author's own feedback, never the commit gate.


**Hand-back.** *Next in the wired run: `hatsu:kokusen` — verify the finished tree and commit it.
...
- **RETIRED at nen `0.5`: validating `nen/workflow.json`.** `nen schema check` carries the row.
  Reading `iteration.checks` is still this skill's (see [`hatsu:breath`](../breath/SKILL.md) § 2),
