...
Which is exactly why [`hatsu:kokusen`](../kokusen/SKILL.md) **re-runs rasengan before each commit**
...
- **Reading `nen/workflow.json`.** No loader and no `nen schema check` row at `0.3.0`; `iteration.checks`
  is read as data by this skill (see [`hatsu:breath`](../breath/SKILL.md) § 2).
