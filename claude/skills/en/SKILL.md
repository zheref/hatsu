---
name: en
description: Take one open pull request from the moment it opens to verified readiness at its human G2/G4 gate, under a mandatory acting-cycle cap — the landing report, fresh current-head observation, Sharingan remediation, catch-up when behind, the bell only at Ready, and Third-Hand's session-closing harvest after the retained report. Use when hatsu:mukai hands over its PR, or when the maintainer invokes hatsu:en on <CODE>#<N> for a PR that is already open. Pending CI or review keeps the run active; quiet polls spend no cycle. En never merges and never casts a review vote.
---
...
and is outside En. The sitting closes with [`hatsu:third-hand`](../third-hand/SKILL.md) after the
...
anything. Starting [`hatsu:third-hand`](../third-hand/SKILL.md) at a terminus is part of that
...
| `notifications.rungs` / `.sound` | `nen/workflow.json` | step 6's rungs — [`hatsu:jutaisho`](../jutaisho/SKILL.md)'s keys | `["push","os","sound"]` / `Glass` |
...
|---|---|---|---|
| 1 | **landing report** | [`hatsu:rikugan`](../rikugan/SKILL.md) `as landing` | once, at the start — 00–07 plus **08 PR body** and **09 Readiness**. If this En was started by mukai in the same sitting with no newer human request, **00** still answers that mukai request |
...
| 7 | **retained readiness report** | [`hatsu:rikugan`](../rikugan/SKILL.md) `as final` | after the bell |
| 8 | **session harvest** | [`hatsu:third-hand`](../third-hand/SKILL.md) | after the retained report, **in parallel** with presenting it; Netero proposes 0–3 folded process issues, the maintainer picks, those are filed, then the sitting is over. Not an acting cycle. The merge stays outside |
...
After the deterministic Ready verdict and step 6's bell, render [`hatsu:rikugan`](../rikugan/SKILL.md)
...
**Then start [`hatsu:third-hand`](../third-hand/SKILL.md) in parallel with presenting that page** —
