...
called**. [`hatsu:mukai`](../mukai/SKILL.md) ends by starting it; from there the PR belongs to en
...
reason [`hatsu:rikugan`](../rikugan/SKILL.md) § 1 records — a lone bracketed slot is refused at the
template.
...
[`hatsu:izanagi`](../izanagi/SKILL.md)'s whole discipline binds: *a loop that writes needs a bound a
...
| `reports.dir` / `.template` / `.retain` | `nen/workflow.json` | step 7's file — [`hatsu:rikugan`](../rikugan/SKILL.md)'s keys | `Reports` / `rikugan` / `final-only` |
| `notifications.rungs` / `.sound` | `nen/workflow.json` | step 5's rungs — [`hatsu:jutaisho`](../jutaisho/SKILL.md)'s keys | `["push","os","sound"]` / `Glass` |
...
> session (§ 7), so **a re-invoked `hatsu:en` on the same PR resumes the same cap rather than
...
|---|---|---|---|
| 1 | **landing report** | [`hatsu:rikugan`](../rikugan/SKILL.md) `as landing` | once, at the start — 01–07 plus **08 PR body** and **09 Readiness** |
...
| 5 | **the bell** | [`hatsu:jutaisho`](../jutaisho/SKILL.md) | **at Ready**, and only at Ready |
| 6 | **watch** | this file, § 6 | until merged — going back to step 2 on a new review or a new conflict, and ringing again |
| 7 | **final report** | [`hatsu:rikugan`](../rikugan/SKILL.md) `as final` | after the merge — the only report kept as a dated file |
...
Steps 2 and 4 are [`hatsu:sharingan`](../sharingan/SKILL.md)'s whole engine, not a substitute for
...
Step 5 is [`hatsu:jutaisho`](../jutaisho/SKILL.md) `at <G2|G4>`, with the gate
...
[`hatsu:izanami`](../izanami/SKILL.md) uses, run **single-shot per cycle**, exactly as
...
|---|---|
| **a new review, comment or thread** | back to step 2 — [`hatsu:sharingan`](../sharingan/SKILL.md) addresses it through its own channel, and **the bell rings again when it returns to Ready** |
...
no self-continuation anywhere in this plane — the same boundary [`hatsu:ren`](../ren/SKILL.md) § 4
...
re-invocation** — `hatsu:en on <CODE>#<N>` re-fetches everything and re-decides from live evidence
...
Step 7 is [`hatsu:rikugan`](../rikugan/SKILL.md) `as final`, after the merge — sections 01–09 plus
...
[`hatsu:getsuga`](../getsuga/SKILL.md)'s, on the maintainer's own call.
