...
Inside [`hatsu:aka`](../aka/SKILL.md) § 6, [`hatsu:tsukuyomi`](../tsukuyomi/SKILL.md) executes each
...
[`hatsu:mukai`](../mukai/SKILL.md) step 6 consumes those existing artifacts for evidence and does
...
[`hatsu:tsukuyomi`](../tsukuyomi/SKILL.md) because a unit test has no golden to overwrite.
...
- **Not a gate event of its own.** Red is reported; the **G5** on a red required suite is
  [`hatsu:aka`](../aka/SKILL.md)'s, raised at the moment a push was about to happen. An unsupported
...
- **Never patches, skips, `.only`-narrows, retry-loops, masks or loosens a comparison** to reach
  green — [`hatsu:tsukuyomi`](../tsukuyomi/SKILL.md) § 9's rule, in this suite's own vocabulary.
