...
Tsukuyomi is **automatic on every [`hatsu:ren`](../ren/SKILL.md) turn that changes executable
behavior**: [`hatsu:rasengan`](../rasengan/SKILL.md) may call it for feedback while authoring, and
[`hatsu:kokusen`](../kokusen/SKILL.md) must call it at the local checkpoint before it commits.
**It is not the project-wide regression.** That run belongs to
[`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md) at [`hatsu:mukai`](../mukai/SKILL.md). Coverage
capture and measurement belong to [`hatsu:byakugan`](../byakugan/SKILL.md), also at mukai. Linting is
[`hatsu:gyo`](../gyo/SKILL.md), on every Ren turn.
...
**Tsukuyomi is normally called by [`hatsu:rasengan`](../rasengan/SKILL.md) for feedback or by
[`hatsu:kokusen`](../kokusen/SKILL.md) at the commit checkpoint, and this section is what it does when
...
|---|---|
| ⭐ **Author the focused tests, then run them** | Hand to [`hatsu:rasengan`](../rasengan/SKILL.md) to write tests for the changed behavior on the stack the declaration names, register the scoped lane where the repository's conventions put one, and return here to run it. Tsukuyomi does **not** write them itself — it is the runner, and a runner that authors its own evidence is not evidence |
...
```
hatsu:tsukuyomi [against <base>] [--lane <explicit-scoped-lane>]
```

```bash
nen parse tsukuyomi --grammar "against [<base>]" --line "<the invocation, minus the hatsu:tsukuyomi prefix and the flags>"
```
...
`hatsu:byakugan`; `nen shu coverage` is its extract verb and reports `met`, never gating.
...
- **RETIRED at nen `0.5`: validating `nen/workflow.json`.** `nen schema check` carries the row
  ([`hatsu:breath`](../breath/SKILL.md) § 2). Tsukuyomi no longer reads `tests.required`; a read it
...
- **Never acts as the project-wide regression inside aka, murasaki, or mukai.** That run is
  [`hatsu:kotoamatsukami`](../kotoamatsukami/SKILL.md)'s.
