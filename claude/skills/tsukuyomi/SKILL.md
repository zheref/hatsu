...
```
hatsu:tsukuyomi [--lane <lane>] [--extra <verb[,verb]>] [--mode <focused|full-regression|diagnostic>]
```
...
- **RETIRED at nen `0.5`: validating `nen/workflow.json`** — `nen schema check` carries the row
  ([`hatsu:breath`](../breath/SKILL.md) § 2). Full-regression mode still reads `tests.required` and
