...
- **CI hardening.** `timeout-minutes` on every job, `concurrency` with cancel-in-progress on the three PR guards and without it on the regenerate builder, a `paths` filter on the mirror check, `ready_for_review` and a draft skip on the guards, all enforced by `scripts/workflow_runner_policy_check.rb`; `templates/pr-readiness.yml` carries the same shape. `scripts/plugin_bump_check.sh` refuses a non-increasing version (a downgrade or an equal version, the way v0.40.0 went untagged), with fixture cases.
