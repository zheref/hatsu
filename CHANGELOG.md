...
- **Every ask reaches the picker.** An ask on the report reaches the surface's own picker in the same turn, never the page alone. Only an ask whose `gate` is G1–G5 makes the bell `jutaisho at <gate>` with the full four-part stop; an `ask-once-per-run` row's ask gets the picker and rung 1, with no banner. The rule is written once in `jutaisho` § 1; `WORKFLOW.md` § 4 (the five G5 stops are still five), `PROCESS.md`, `claude/rules/hatsu.md` and `ren`'s step 6 cite it.
- **Ask, set up, continue** (`WORKFLOW.md` § 4). A missing argument or configuration item no longer ends a skill. It is derived for this run only; otherwise it is asked for per item through the picker, written through its owner once the maintainer answers (nothing tracked is written unasked), validated, and the operation resumes (`missing-argument`, `missing-configuration`). A new lane, verb `argv` or `project.targets` entry lands through its declaration PR at that repository's gate before it runs. A value that looks like a secret is never written: `requiresEnv`-style keys take environment-variable names only, `nen stage triage` runs on the written file, and the `missing-configuration` row carries that refusal as data. A frozen or foreign registry is reported, never written (`senkei` § 1). What is not a gap is listed once, in § 4 *What is not a gap*.
- **The maintainer's word is typed, never picked.** A new `nen/decisions.json` row, `missing-maintainer-choice` (`ask-once-per-run`, no `default`, no recommended option, the keys it governs listed in `governs`), covers a deploy or release target, a G3 go, an iteration cap, a device and ren's missing request. Declared candidates may be listed for reference, none starred and none a picker option that performs the act. `kagutsuchi`, `mugetsu`, `izanagi`, `jujutsu` and `ren` cite it. `mugetsu`'s P4 is restored ("does not prompt for the go … does not present publication as an option in any picker"): it asks for a missing part of a go only when the maintainer typed `hatsu:mugetsu <target>` in the same turn, every other reach stops without relaying an ask, and an undeclared target is refused, naming the declaration PR that would add it. `missing-argument`'s default now says it does not cover a maintainer's-word value.
- **`docs/PROCESS.md`** holds the conventions every phase shares, so the seventeen capped skills each stay under 11,000 bytes (`scripts/prose_size_check.sh` still enforces 12,288). Both it and `WORKFLOW.md` state one precedence: a skill's own hard limit, then `WORKFLOW.md`, then `PROCESS.md`, which binds wherever the skill says nothing. New in it: *Reviewer rounds and review threads* (Copilot through `--add-bots`, thread hygiene, when a round is complete), moved out of `sharingan` and `shibari`; the live named-raw-call list under *Residue and owned dependencies*, by skill; and an owned dependency on zheref/nen#258, since `nen report data` does not derive `effortStage`, `gate`, `turnLabel`, `worktree` or `generatedAtLocal`, and stamps `repo` from the worktree directory. `docs/SURFACES.md` joins the shared-policy header.
- **`hatsu:amenotejikara`** swaps a worktree's committed tree into the core checkout, where Xcode and `amaterasu` build it, and puts core back with its own work parked in a pinned commit, never stashed, on `nen wc swap` and `nen wc worktrees`. A failed step recovers through `nen wc swap --status` and then `--return`, never raw `git checkout`. It has a `## 0. Standalone entry` (P1 is required, since nen `0.14` is) and is classified in `STANDALONE-ENTRY.md` § 7b. `amaterasu` § 3 resolves core from `nen wc worktrees` and names or refuses an active swap; `breath` § 2 refuses to warm core while one is active.
- **The evidence table, checked.** Every screenshot in a PR body is a cell of the UZF-26 evidence table, and `scripts/pr_body_evidence_check.sh` refuses a loose or unlabeled image before every body write. It reads the body on stdin (a path containing `=` is not an awk assignment), refuses a directory, detects reference-style `![alt][ref]` images and `<img` tags split across lines, ignores inline code, treats an unclosed `<table>` as no table, and counts a markdown row only inside a table with a `|---|` separator. Every skill and `templates/pr-body.md` spell both scripts as `"$hatsu_root/scripts/<name>"`.
- **Two new lanes**, `report-time-guard` and `pr-body-guard`, run each script's `--self-test`. `scripts/plugin_bump_check.sh` now covers `docs/PROCESS.md`, `scripts/report_time.sh` and `scripts/pr_body_evidence_check.sh`, with a fixture case for each.
- Counts: forty-four skills plus `hatsu-warmup` (forty-five directories per surface); the plugin manifest at 0.45.0.
