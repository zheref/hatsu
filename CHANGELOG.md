...
> **`v0.37.0` is held by [#77](https://github.com/zheref/hatsu/pull/77) on `claude/frosty-stonebraker-3f2b1a`, still open** — two branches cannot claim one bump. Taking the next number means this can merge in either order. As `v0.36.0`'s note records, `scripts/plugin_bump_check.sh`'s `version_bumped` asks only whether HEAD's `version` DIFFERS from base's, never whether it is HIGHER, so the ordering is still coordinated by hand and **a guard rejecting a non-increasing bump is still owed**.
