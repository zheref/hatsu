...
mark_adopted() {
  ...
  # (`GENERATED by nen surface mirror (surface: codex, stamp: 0.43.0)`).
  # Codex and Antigravity share `.agents/`, so a marker with no surface name
  # would (wrongly) authorize either one's unattended full-surface install
  # off the other's adoption.
  ...
  mkdir -p "$marker_dir"
  printf 'GENERATED by nen surface mirror (surface: %s, stamp: fixture)\n' "$surface" > "$marker_dir/SKILL.md"
}
...
# The PLACED copy's root expression is rewritten for workspace mode (its
# hook scripts land right beside it, at .agents/hooks/) -- so it must NOT be
# byte-identical to the tracked mirror, which keeps the generated,
# global-plugin-defaulting form (surfaces/antigravity/hooks.json).
if cmp -s "$antigravity_fixture/.agents/hooks.json" "$hatsu_root/surfaces/antigravity/hooks.json"; then
  fail "Antigravity placed hooks.json was not rewritten for workspace mode (still byte-identical to the tracked, global-defaulting mirror)"
fi
grep -qF '${HATSU_PLUGIN_ROOT:-./.agents}' "$antigravity_fixture/.agents/hooks.json" || fail "Antigravity placed hooks.json commands do not fall back to ./.agents"
...
# (nen#233 follow-up) Codex and Antigravity share `.agents/` as their marker
# directory. A repository that has adopted Codex only -- a marker naming
# `(surface: codex, …)` and nothing naming antigravity -- must NOT pass
# Antigravity's own adoption gate: one surface's adoption never authorizes
# another's unattended full-surface install.
codex_only_fixture="$fixture_root/codex-only-not-antigravity"
new_fixture "$codex_only_fixture"
mark_adopted "$codex_only_fixture" codex
codex_only_output="$("$bootstrap" --surface antigravity --target "$codex_only_fixture" --install-all 2>&1)"
case "$codex_only_output" in
...
new_fixture "$antigravity_only_fixture"
mark_adopted "$antigravity_only_fixture" antigravity
antigravity_only_output="$("$bootstrap" --surface codex --target "$antigravity_only_fixture" --install-all 2>&1)"
case "$antigravity_only_output" in
